             ...
             const std::vector<Triplet>& triplets)
{
    ...
    uint32_t num_barcodes = static_cast<uint32_t>(barcodes.size());
    ...
    for (const auto& t : triplets) {
        if (t.cell_idx < barcodes.size() && t.gene_idx < features.size()) {
    ...
    // Write barcodes (ordered by column)
             ...
             const std::vector<Triplet>& triplets)
{
    ...
    return writeMex(outputPrefix, barcodes, features, triplets);
}
